...
void* process_data_parallel(void* arg) {
    vector<vector<Node*>>& nodes_array = *(vector<vector<Node*>>*) arg;
    double min_aic = INFINITY;
    vector<string> min_data;
    double aic; vector<string> array_id;
    
    for (int i = 0; i < nodes_array.size(); i++) {
        vector<Node*> combination = nodes_array[i];
...
int main() {
    ...
    */
    ...
    for (int i = 1; i <= size; i++) {
        final_result[i-1] = combinations(node_array, i);

        int create_result = pthread_create(&threads[i-1], nullptr, process_data_parallel, &final_result[i-1]);
        if (create_result != 0) {
